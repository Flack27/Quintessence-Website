...
namespace QuintessenceWebsiteDAL.DAL
{
    ...
    /// <summary>Main roster (static, admin-managed), backed by a JSON file.</summary>
    public class RosterDAL : IRosterDAL
    {
        private readonly JsonStore<RosterMemberDTO> _store;

        public RosterDAL(JsonStore<RosterMemberDTO> store)
        {
            _store = store;
        }

        public List<RosterMemberDTO> GetMembers() =>
            _store.Read().OrderBy(m => m.DisplayOrder).ThenBy(m => m.MemberId).ToList();

        public RosterMemberDTO AddMember(RosterMemberDTO member) =>
            _store.Update(list =>
            {
                member.MemberId = list.Count == 0 ? 1 : list.Max(m => m.MemberId) + 1;
                member.DisplayOrder = list.Count == 0 ? 0 : list.Max(m => m.DisplayOrder) + 1;
                list.Add(member);
                return member;
            });

        public bool UpdateMember(RosterMemberDTO member) =>
            _store.Update(list =>
            {
                var i = list.FindIndex(m => m.MemberId == member.MemberId);
                if (i < 0) return false;
                list[i] = list[i] with
                {
                    ...
                    GameIds = member.GameIds
                };
                return true;
            });

        public bool DeleteMember(long memberId) =>
            _store.Update(list => list.RemoveAll(m => m.MemberId == memberId) > 0);

        public bool ReorderMembers(List<long> orderedMemberIds) =>
            _store.Update(list =>
            {
                for (int pos = 0; pos < orderedMemberIds.Count; pos++)
                {
                    var i = list.FindIndex(m => m.MemberId == orderedMemberIds[pos]);
                    if (i >= 0) list[i] = list[i] with { DisplayOrder = pos };
                }
                return true;
            });
